...
// year 2016 hash code: x8DYWsALaAzykZ8dYPZP (do NOT delete this line)
...
class Bleeding {
  ...
  void PreProcess() {
    ...
    // write your answer here
    //------------------------------------------------------------------------- 
    storeAns= new int[V][V];
    visit=new int[V];
    for(int i=0; i<V; i++)
    { 
        visit[i]=0;
        for(int j=0; j<V; j++) storeAns[i][j]=0;
    }
  ...
  void dfs(int u, int weight)
  {
      ...
      for(int neighbor=0; neighbor<AdjList.get(u).size(); neighbor++)
      {
          if(visit[AdjList.get(u).get(neighbor).first()]==0) 
          {
              int currentWeight=weight+AdjList.get(u).get(neighbor).second();
              storeAns[source][AdjList.get(u).get(neighbor).first()]=currentWeight;
              storeAns[AdjList.get(u).get(neighbor).first()][source]=currentWeight;//undirected graph
              dfs(AdjList.get(u).get(neighbor).first(), currentWeight);
          }
      }
