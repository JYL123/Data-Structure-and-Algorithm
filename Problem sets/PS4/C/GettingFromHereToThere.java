...
class GettingFromHereToThere {
  ...
  private ArrayList<IntegerPair> selectedEdges;
  ...
  int Query(int source, int destination) {
    ...
    selectedEdges=new ArrayList<IntegerPair>();
    ...
    while(!pq.isEmpty())
    {
        ...
        {
            selectedEdges.add(front);
            break;
        }
        else if(!taken.get(front.second()))
        {
            selectedEdges.add(front);
    ...
    ans=selectedEdges.get(0).first();
    for(IntegerPair p: selectedEdges)
    {
        //System.out.println("vertex is "+p.second()+" weight is "+p.first());
